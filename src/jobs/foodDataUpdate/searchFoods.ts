...
const normalizeName = (rawName: string) : { n: string; amt: string | null} => {
    ...
    n = n
		...
		// 乳製品などのブランド・社名
		.replace(/マリンフード|帝国ホテルキッチン|帝国ホテル|CasaRADICCI|Colla|イソラビオ|アキタ|岩泉ディングス|乳業|小岩井|アルティ|ロスフェルド|ヴァーニュ|プレジデント|丸和油脂|ホテルオークラ|日本酪農協同|オハヨー/g, '')
		...
		.replace(/ニュータッチ|凄麺|マルタイ|クノール|伊藤園|UCC|マルサンアイ|マルサン|ソイフーズ|六甲バター|男前豆腐店|GREENDAKARA|ダカラ|フーズ/g, '')
		...
		.replace(/日本珈琲貿易|日本フェレロ|フェレロ|ブライトザマー|Maple\s*Treat|LB\b/g, '')
		// 個別商品・飲料系の寄せ
		.replace(/朝のフレッシュ\s*糖質?/g, 'コーヒーホワイトナー 液状 植物性脂肪')
		.replace(/ポップアップス!?/g, 'ロリポップ')
		.replace(/ポッピングドロップ/g, 'ドロップ')
		.replace(/ロリポップジェリー/g, 'ゼリーキャンデー')
		.replace(/パラソルカラーチョコ/g, 'チョコレート')
		.replace(/大地の/g, '')
		.replace(/濃密/g, '')
		.replace(/カラダの/g, '')
		.replace(/幼児優良牛乳/g, '普通牛乳')
		...
		.replace(/ブレッド/g, '食パン')
		.replace(/パンにおいしい/g, '')
		.replace(/しあわせのもっちり仕込み(?:\s*山型食パン)?/g, '食パン')
		.replace(/山型食パン/g, '食パン')
		.replace(/オリーブオイルをつかったパン/g, 'ロールパン')
		.replace(/ナポリタンロール|塩焼きそばロール/g, 'ロールパン')
		.replace(/うしりコロッケパン/g, 'コロッケパン')
		.replace(/チョリームパン/g, 'クリームパン')
		.replace(/つぶあんぱん/g, 'あんパン')
		// 炭酸飲料・コーラ
		...
		.replace(/タイ\b/g, '')
		.replace(/ノルウェー|イギリス|ロシア/g, '')
		.replace(/又は/g, '')
		.replace(/[＋+]\s*MCT/gi, '')
		.replace(/Ca鉄/gi, '')
		.replace(/ハロウィン|キャニスター缶|スクエア缶|コスチュームベアー|パンプキン|蓄光|ジッパーバッグ|バッグ|ポット|チューブ/g, '')
		.replace(/すみっコぐらし|しろくま|ねこ|とかげ|ミッキー/g, '')
		.replace(/ザナッツ|まいにち食べたい|太陽と実/g, '')
		.replace(/ダーボン|オキス|美訓物産|企業連合|屋仙七|まるせん|五十鈴|フォスター|竹葉堂|リン|ストライキング|宇治の露製茶|伊右衛門|芥川|森半|金城|川口|川光商事|露園|カモ井|クラウンフーヅ|ユウキ食品|マコーミック|MJB|南国|谷食糧工業|乃川島|ウィズメタック|MAKS|ラカサ|アケラス/g, '')
		.replace(/万能の素|万能/g, '')
		.replace(/[ヶヵカケ]月/g, '')
		// 産地・原産国・先頭ノイズの除去
		...
		.replace(/減塩/g, '')
		.replace(/原材料名/g, '')
		.replace(/選ばれし人気店|料亭の味|具だくさん|てりやき|純粋|風味豊かな|製菓/g, '')
		.replace(/食べられる/g, '')
        ...
        .replace(/キクイモ茶/g, 'せん茶 浸出液')
		// コーヒー乾燥系
		.replace(/フリーズドライ(?:袋)?/g, '')
		.replace(/ドリップオン|トアルコ|トラジャ/g, '')
		// イースト・ビネガー・トリュフ
		.replace(/日仏商事/g, '')
		...
		.replace(/シャキッとコーン(?:\s*缶)?/g, 'スイートコーン')
		.replace(/ジューシーぶどうゼリー/g, 'ゼリー')
		.replace(/温泉たまご|温泉卵/g, '鶏卵 全卵 生')
		.replace(/たむらのタマゴ/g, '鶏卵 全卵 生')
		.replace(/うずらの恵/g, '')
		.replace(/旬味香彩/g, '')
		.replace(/みぶな/g, 'みずな')
		.replace(/セレクトスイーツ/g, '')
		...
		.replace(/ギリシャヨーグルト/g, 'ヨーグルト 全脂無糖')
		.replace(/ヨーグルト(?!ケーキ)/g, 'ヨーグルト 全脂無糖')
		// ヨーグルト関連のブランド・表記ゆれまとめ
		.replace(/(?:ダノン|オイコス|ビヒダス|ビオ|プロビオ|ナチュレ\s*恵|megumi|ガセリ菌SP|MBPドリンク|トリプルヨーグルト|岩泉ヨーグルト|岩泉ディングス)[^\n]*/g, 'ヨーグルト 全脂無糖')
		.replace(/ココナッツミルクヨーグルト|豆乳ヨーグルト|豆乳グルト|オーツミルクヨーグルト|な豆乳ヨーグルト/g, 'ヨーグルト 全脂無糖')
		.replace(/のむヨーグルト|飲むヨーグルト|ドリンクタイプ/g, '')
		.replace(/脂肪ゼロ|低脂肪|無脂肪/g, '')
		.replace(/砂糖不使|砂糖不使用/g, '')
		.replace(/[A-Z]{1,3}[ー－-]/g, '')
		.replace(/(ベリー|シチリア|アサイー|アロエ|いちご|ぶどう|ストロベリー|マンゴー|みかん|アップル)\b/g, '')
		.replace(/糖質off|糖質オフ/gi, '')
		.replace(/至福のギリシャ/g, 'ヨーグルト 全脂無糖')
		.replace(/パルテノ|クリーミーバニラ味|いちごソース|ベリーソース/g, '')
		...
		.replace(/にわとり\s*むね\s*生/g, 'にわとり むね 皮なし 生')
		.replace(/鴨/g, 'かも')
		...
		.replace(/加工品/g, '')
		.replace(/仕立て/g, '')
		.replace(/(ごまみそ|しょうが|トマト)煮/g, '')
		.replace(/トマト味|トマトソース/g, '')
		.replace(/とびうお\s*卵/g, 'とびうお 卵')
		.replace(/焼いわし/g, 'いわし 焼き')
		.replace(/いわし\s*油漬\s*いわし\s*油漬/g, 'いわし 油漬')
		.replace(/焼き?だいこんおろし\s*梅?/g, '')
		.replace(/イクラ/g, 'いくら')
		.replace(/蒸しだこ/g, 'たこ')
		.replace(/ぎんざけチーズ味/g, 'ぎんざけ')
		...
		.replace(/タコライスソース/g, '調味ソース')
		.replace(/しょうが蜜/g, 'シロップ')
		.replace(/スチームあさり身/g, 'あさり')
		...
		.replace(/インスタントオーツ(?:オートミール)?/g, 'オートミール')
		...
		.replace(/ミモレット/g, 'ナチュラルチーズ ミモレット')
		.replace(/ブラータ/g, 'ナチュラルチーズ ブラータ')
		.replace(/コンテ/g, 'ナチュラルチーズ コンテ')
		.replace(/パルミジャーノ[・\s-]?(?:レッジャーノ|レジャーノ)/g, 'ナチュラルチーズ パルメザン')
		.replace(/レッド?\s*チェダー|ホワイト\s*チェダー/g, 'ナチュラルチーズ チェダー')
		.replace(/ブリー/g, 'ナチュラルチーズ カマンベール')
		.replace(/モザレラ|モッツアレラ/g, 'ナチュラルチーズ モッツァレラ')
		.replace(/ブロック/g, '')
		...
		.replace(/かけるチーズ/g, 'ナチュラルチーズ パルメザン')
		.replace(/チーズLuxe/g, 'ナチュラルチーズ')
		// 卵・乳酸菌飲料
		...
		.replace(/レーズン(?:アンド)?/g, '干しぶどう')
		.replace(/カリフォルニア干しぶどう/g, '干しぶどう')
		.replace(/種抜き/g, '')
		.replace(/小豆の赤飯/g, '赤飯')
		...
		.replace(/ヌテラ/g, 'チョコレートスプレッド')
		.replace(/スキッピー|SKIPPY/g, '')
		.replace(/ピーナツ|ピーナッツ/g, 'ピーナッツ')
		.replace(/ピーナッツ\s*(クリーミー|チャンク|クランチ|バター)/g, 'ピーナッツバター')
		.replace(/メープル\s*(?:シロップ)?\s*(?:ダーク|グレードA|アンバ)?/g, 'メープルシロップ')
		.replace(/アガベシロップ/g, 'シロップ')
		.replace(/アカシアハニー|マヌカハニー|アカシア\b/g, 'はちみつ')
		...
		.replace(/ホテルマーガリン/g, 'マーガリン')
		.replace(/マーガリン[^\s]*/g, 'マーガリン')
		.replace(/餃子の皮/g, 'ぎょうざ')
		...
		.replace(/ミューズリー|フルーツミューズリー|フルーツナッツミューズリー/g, 'オートミール')
