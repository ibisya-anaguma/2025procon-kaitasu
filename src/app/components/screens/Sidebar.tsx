  ...
  const renderSidebarButton = (
    ...
    return (
      <Button
        ...
        variant="ghost"
        size="sm"
        className="w-full h-[99.27px] flex justify-end px-0 py-0 bg-transparent hover:bg-transparent shadow-none focus-visible:ring-0 focus-visible:ring-offset-0"
        onMouseEnter={() => setHoveredNav(key)}
        onMouseLeave={() => setHoveredNav(null)}
        onClick={() => router.push(href)}
        data-oid={dataOid}>
        <span className="flex items-center justify-end w-full h-[99.27px] shrink-0">
          <span className="flex items-center relative w-[192px] h-[99.27px] shrink-0 bg-no-repeat bg-[length:100%_100%] bg-[right_center] justify-start pl-5 gap-3 translate-x-px">
            <span
              className="absolute inset-0 bg-no-repeat bg-[length:100%_100%] bg-center pointer-events-none z-0"
              style={{
                backgroundImage: showHighlight ? SIDEBAR_ACTIVE_BG : "none",
                opacity: highlightOpacity
              }}>
            </span>
            <span className="flex w-[31.5px] h-[34.616px] shrink-0 justify-center items-center relative z-[1]">
              <Icon fill={iconFill} stroke={iconStroke} />
            </span>
            <span
              className={`flex w-[118px] h-[52.616px] justify-center items-center gap-[4.5px] text-center font-['BIZ_UDPGothic'] text-[20px] font-bold leading-[100%] tracking-[1.04px] relative z-[1] ${
